...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt): #初始化
        ...
        size = opt.fineSize
        self.opt = opt
        self.input_A = self.Tensor(nb, opt.input_nc, size, size) #代增强
...
#定义感知损失
        if opt.vgg > 0:
            self.vgg_loss = networks.PerceptualLoss(opt) #全局
            if self.opt.IN_vgg:
                self.vgg_patch_loss = networks.PerceptualLoss(opt) #局部
...
#定义网络
        ...
        if not self.isTrain or opt.continue_train: #加载训练好的权重
    ...
    def set_input(self, input): #加载数据
        AtoB = self.opt.which_direction == 'AtoB'
        input_A = input['A' if AtoB else 'B']
        input_B = input['B' if AtoB else 'A']
        input_img = input['input_img']
        input_A_gray = input['A_gray']
        #d_network = light.DNetwork()
        #A_img = input_img.detach().unsqueeze(0)
        #l_img = d_network.forward(A_img)
        #l_img = l_img.detach().squeeze(0)
        #input_img = mask.auto_illu_mask(input_img, 0.1, 0.2, 2, 3)
    ...
    def forward(self):
        ...
        if self.opt.input_linear:
            self.real_A = (self.real_A - torch.min(self.real_A))/(torch.max(self.real_A) - torch.min(self.real_A))
        if self.opt.skip == 1:#最后输出是否有跳层链接
...
#取图像块
        ...
        if self.opt.patchD_3 > 0:#多个图像块
            ...
            #        w_offset_2:w_offset_2 + self.opt.patchSize]
#生成器损失
    def backward_G(self, epoch):#反向传播
        pred_fake = self.netD_A.forward(self.fake_B)
        if self.opt.use_wgan:
            self.loss_G_A = -pred_fake.mean()
        elif self.opt.use_ragan: #相对GAN损失
            ...
            self.loss_G_A = self.criterionGAN(pred_fake, True) #交叉熵
        ...
        if self.opt.vgg > 0:#感知损失
            ...
            self.loss_G = self.loss_G_A + self.loss_fcn_b*vgg_w#完整生成器的损失
        # self.loss_G = self.L1_AB + self.L1_BA

        use_static_mask = self.opt.use_static_mask
        # static mask
        if use_static_mask:
            static_mask = self.get_static_mask(self.fake_B, self.real_A)
            self.loss_G *= static_mask
            self.loss_G.append(self.loss_G)
    ...
    def optimize_parameters(self, epoch):#优化函数
        # forward
        self.forward()
        # G_A and G_B
        self.optimizer_G.zero_grad()#梯度清0
        self.backward_G(epoch) #反向传播
        self.optimizer_G.step()
        # D_A
        self.optimizer_D_A.zero_grad()
        self.backward_D_A()
        if not self.opt.patchD:#局部
            self.optimizer_D_A.step()
        else:
            # self.forward()
            self.optimizer_D_P.zero_grad()
            self.backward_D_P()
            self.optimizer_D_A.step()#全局判别器参数更新
            self.optimizer_D_P.step()


    def get_current_errors(self, epoch):#获取当前的损失函数值
