...
def pad_tensor(input): #将输入张量的高度和宽度调整为接近且是指定值的整数倍
    
    height_org, width_org = input.shape[2], input.shape[3]
    divide = 16 # divide表示填充后的高度和宽度应该是原始高度和宽度的整数倍

    if width_org % divide != 0 or height_org % divide != 0: #不能整除，填充
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, gpu_ids=[], skip=False, opt=None):
    ...
    elif which_model_netG == 'sid_unet_resize':#用
...
class Unet_resize_conv(nn.Module):
    ...
    def forward(self, input, gray):#输入 ，注意力图
        flag = 0
        if input.size()[3] > 2200: #尺寸
            avg = nn.AvgPool2d(2)
            input = avg(input)
            gray = avg(gray) #平均池化
            flag = 1
            # pass
        input, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(input) #填充
        gray, pad_left, pad_right, pad_top, pad_bottom = pad_tensor(gray)
        if self.opt.self_attention: #得到各级注意力图
        ...
        if self.opt.use_norm == 1:
            ...
            conv5 = self.bn5_2(self.LReLU5_2(self.conv5_2(x)))  #编码
            
            conv5 = F.upsample(conv5, scale_factor=2, mode='bilinear') #解码
            ...
            if self.opt.times_residual: #残差
                latent = latent*gray

            # output = self.depth_to_space(conv10, 2)
            if self.opt.tanh:#是否跳层链接
            ...
            if self.skip:
                if self.opt.linear_add:
                    ...
                    if self.opt.latent_threshold:
                        ...
                        latent = (latent - torch.min(latent))/(torch.max(latent)-torch.min(latent))#归一化
                    output = latent + input*self.opt.skip
            else:
                output = latent

            if self.opt.linear: #是否线性映射
                output = output/torch.max(torch.abs(output))
            
                
        elif self.opt.use_norm == 0: #是否归一化
            if self.opt.self_attention:
